...
def deal_with_page(html):
    ...
    for block in file_split:
        ...
        if len(block_split) < 2:
            continue
        if (len(block_split[0])<10):
            continue

        if (len(block_split[0])>10):
            continue

        if (block_split[0][0] != 'B'):
            continue
        #price
        price_low = 0
        price_high = 0
...
def getcookie():
    ...
    c1 = parse_cookie(cookie_file1)
    c2 = parse_cookie(cookie_file2)
    c3 = parse_cookie(cookie_file3)
...
if __name__ == "__main__":
    ...
    filename = '/Users/yipengm/PycharmProjects/AMZ_LIB/word_' + mode
    f = open(filename)
    ...
    driver = webdriver.Chrome(
        '/Users/yipengm/Downloads/chromedriver',options=options)  # 创建一个driver用于打开网页，记得找到brew安装的chromedriver的位置，在创建driver的时候指定这个位置
    ...
    for k, v in search_dict.items():
        ...
        while i < 7:
            ...
            url = 'https://www.amazon.com/s?k=' + kw_reassemble + '&page='+str(i+1)
            driver.get(url)
            ...
            if ASIN_arr[0][0] == 1 or ASIN_arr[1][0] == 1 or adstuck_flag > 1:
                remote_path = '/E:/luosi_data/' + date.replace(':','_') + '/' + mode + '/' + kw_reassemble + '_collect/'
